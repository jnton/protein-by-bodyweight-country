const DATA_URL = "./data/protein_bodyweight_country_year.json";
...
function commonLayout() {
  return {
    ...
    margin: {l: 54, r: 20, t: 18, b: 50},
    hoverlabel: {font: {family: "Inter, system-ui, sans-serif"}}
  };
}

function recordsForYear(year, requireMetric = true) {
  return state.records.filter((row) => Number(row.Year) === Number(year) && (!requireMetric || row[state.metric] != null));
}
...
function updateYearRange() {
  ...
  slider.max = String(years.at(-1));
  if (!years.includes(state.year)) state.year = Number(years.at(-1));
...
function renderSummary() {
  ...
  const years = [...new Set(normalized.map((row) => Number(row.Year)))].sort((a, b) => a - b);
  const countries = new Set(normalized.map((row) => row.Code));
  el("country-count").textContent = countries.size.toLocaleString();
  el("record-count").textContent = state.records.length.toLocaleString();
  el("year-range").textContent = years.length ? `${years[0]}–${years.at(-1)}` : "—";

  const slider = el("year-slider");
  slider.min = String(years[0]);
  slider.max = String(years.at(-1));
  state.year = Number(years.at(-1));
  slider.value = String(state.year);
  el("year-output").value = String(state.year);
}
...
function populateCountrySelect() {
  ...
  state.records.forEach((row) => {
    if (row.Code && row.Entity) countryMap.set(row.Code, row.Entity);
  });
  const options = [...countryMap.entries()].sort((a, b) => a[1].localeCompare(b[1]));
  const select = el("country-select");
  select.innerHTML = options.map(([code, name]) => `<option value="${code}" ${state.selectedCodes.includes(code) ? "selected" : ""}>${name}</option>`).join("");
}
...
function renderMap() {
  ...
  const trace = {
    ...
    customdata: rows.map((row) => [row.protein_supply_g_day, row.estimated_adult_bodyweight_kg, row.estimate_status]),
    ...
    hovertemplate: `<b>%{text}</b><br>${definition.label}: %{z:.${definition.decimals}f} ${definition.unit}<br>Protein supply: %{customdata[0]:.1f} g/person/day<br>Adult bodyweight proxy: %{customdata[1]:.1f} kg<extra></extra>`
  };
...
function renderRanking() {
  ...
  const rows = recordsForYear(state.year)
    ...
    .slice(0, 12);
  el("ranking-list").innerHTML = rows.map((row, index) => `
    <li>
...
function renderTrend() {
  ...
  const rows = state.records.filter((row) => selected.has(row.Code) && row[state.metric] != null);
  const byCountry = Map.groupBy ? Map.groupBy(rows, (row) => row.Code) : rows.reduce((map, row) => {
    if (!map.has(row.Code)) map.set(row.Code, []);
    map.get(row.Code).push(row);
    return map;
  }, new Map());

  const traces = [...byCountry.values()].map((countryRows) => {
    const sorted = countryRows.sort((a, b) => Number(a.Year) - Number(b.Year));
    return {
      type: "scatter",
      mode: "lines",
      name: sorted[0].Entity,
      x: sorted.map((row) => row.Year),
      y: sorted.map((row) => row[state.metric]),
      line: {width: 2.4},
      hovertemplate: `<b>${sorted[0].Entity}</b><br>%{x}: %{y:.${definition.decimals}f} ${definition.unit}<extra></extra>`
    };
  });
...
function renderScatter() {
  const rows = recordsForYear(state.year, false).filter((row) => row.protein_supply_g_day != null && row.estimated_adult_bodyweight_kg != null);
  const trace = {
    type: "scatter",
    mode: "markers",
...
function wireEvents() {
  ...
  el("country-select").addEventListener("change", (event) => {
    state.selectedCodes = [...event.target.selectedOptions].map((option) => option.value).slice(0, 8);
    renderTrend();
  });
